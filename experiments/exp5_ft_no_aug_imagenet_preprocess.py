...
def main():
    ...
    exp_name = "exp5_ft_no_aug_imagenet_preprocess"

    # ResNet용 DataLoader (augmentation=False)
    ...
    for epoch in range(num_epochs):
        ...
        print(f"[Exp5][{epoch+1:02d}/{num_epochs}] Train Acc: {tr_acc:.4f} | Val Acc: {va_acc:.4f}")

    if best_state is not None:
        model.load_state_dict(best_state)

    test_loss, test_acc = evaluate(model, test_loader, criterion, device)
    print(f"[Exp5] Best Val Acc: {best_val_acc:.4f}")
    print(f"[Exp5] Test Acc: {test_acc:.4f}")
